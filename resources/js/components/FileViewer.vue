  ...
  <div>
    ...
    <table v-if="strings.length > 0" class="table table-hover table-sm mb-0">
      ...
      <tr v-for="(string, key) in strings" @click="openEditor(string, key)"
          :class="{'table-active':selectedRow==key, 'table-success':string.saved==true, 'table-danger':string.failed==true, 'table-dark':string.obsolete==true, 'table-warning':string.fuzzy==true}">
        <td>
          <div class="msg-id">
